...
const COLUMNS = (hideClientColumn: boolean, hidePricing: boolean) => [
...
function fillJobsSheet(sheet: ExcelJS.Worksheet, jobs: JobRow[], hideClientColumn: boolean, hidePricing: boolean) {
  sheet.columns = COLUMNS(hideClientColumn, hidePricing);
  ...
  jobs.forEach((j) => {
    ...
    if (!hidePricing) MONEY_COLS.forEach((key) => { row.getCell(key).numFmt = '#,##0.00'; });
  ...
  // Grand total row is inherently a pricing figure -- skipped entirely
  // when pricing is hidden, not just its value blanked out.
  if (!hidePricing && jobs.length > 0) {
...
export async function buildJobsWorkbook(
  jobs: JobRow[],
  opts?: { hideClientColumn?: boolean; includeSummary?: boolean; hidePricing?: boolean }
): Promise<Buffer> {
  ...
  const hidePricing = !!opts?.hidePricing;
  // Summary is entirely revenue/pricing analysis -- forced off whenever
  // pricing itself is hidden, regardless of what was passed in.
  const includeSummary = !hidePricing && opts?.includeSummary !== false;
  ...
  if (jobs.length === 0) {
    ...
    fillJobsSheet(sheet, [], hideClientColumn, hidePricing);
  } else {
    [...byMonth.keys()].forEach((key) => {
      const sheet = wb.addWorksheet(monthLabel(key));
      fillJobsSheet(sheet, byMonth.get(key)!, hideClientColumn, hidePricing);
    });
  }
